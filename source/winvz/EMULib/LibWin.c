...
/*************************************************************/
#define VIDEO_SYNC   0x0001
#define VIDEO_TV     0x0002
#define VIDEO_SOFTEN 0x0004
...
static unsigned int VideoMode;    /* ORed VIDEO_* bits       */
...
static HWAVEOUT hWave;            /* WaveOut device handle   */
static DWORD ThreadID;            /* Audio thread ID         */
static int CurHdr;                /* Next header to output   */
static int SndSize;               /* SndBuffer[] size        */
static int SndRate;               /* Sound playback rate     */
static WAVEHDR SndHeader[SND_HEADERS]; /* WaveOut headers    */
static sample *SndBuffer;         /* Ring buffer for audio   */
...
int ShowVideo(void)
{
  ...
  if((VideoMode&(VIDEO_SOFTEN|VIDEO_TV))||(DDFullScreen==2))
  {
    ...
    if(!BigScreen.Data)
    {
      ...
      W = (AppWidth>W)||(VideoMode&VIDEO_SOFTEN)? W
        : (W/AppWidth)*AppWidth;
      H = !DDFullScreen? AppHeight:DDFullScreen==2? 480:240;
      H = (AppHeight>H)||(VideoMode&VIDEO_SOFTEN)? H
  ...
  else
  {
    ...
    if(VideoMode&VIDEO_TV) TelevizeImage(Img,VideoX,VideoY,VideoW,VideoH);
  }

  /* If timer synchronization requested, wait for timer */
  if(VideoMode&VIDEO_SYNC) WaitSyncTimer();
...
int PASCAL WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR CmdLine,int CmdShow)
{
  ...
  VideoMode  = WinGetInteger(0,"Effects",VideoMode);
//  FullScreen = WinGetInteger(0,"FullScreen",FullScreen);
  ...
  CheckMenuItem(GetMenu(hWnd),WMENU_SOFTEN,VideoMode&VIDEO_SOFTEN? MF_CHECKED:MF_UNCHECKED);
  CheckMenuItem(GetMenu(hWnd),WMENU_TV,VideoMode&VIDEO_TV? MF_CHECKED:MF_UNCHECKED);
  CheckMenuItem(GetMenu(hWnd),WMENU_SYNC,VideoMode&VIDEO_SYNC? MF_CHECKED:MF_UNCHECKED);
  ...
  switch(Msg)
  {
    ...
    case WM_PAINT:
      ...
      if(VideoImg&&VideoImg->Data)
      {
        ...
        if(BigScreen.Data&&((VideoMode&(VIDEO_SOFTEN|VIDEO_TV))||(DDFullScreen==2)))
    ...
    case WM_DESTROY:
      ...
      WinSetInteger(0,"Effects",VideoMode);
    ...
    case WM_COMMAND:
      switch(LOWORD(wParam))
      {
        ...
        case WMENU_TV:
          ...
          break;
        case WMENU_SOFTEN:
          VideoMode^=VIDEO_SOFTEN;
          CheckMenuItem(GetMenu(hWnd),WMENU_SOFTEN,VideoMode&VIDEO_SOFTEN? MF_CHECKED:MF_UNCHECKED);
          break;
        case WMENU_SYNC:
          VideoMode^=VIDEO_SYNC;
          CheckMenuItem(GetMenu(hWnd),WMENU_SYNC,VideoMode&VIDEO_SYNC? MF_CHECKED:MF_UNCHECKED);
          break;
        case WMENU_QUIT:
          DestroyWindow(hWnd);
